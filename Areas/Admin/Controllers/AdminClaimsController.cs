...
namespace MvcWebIdentity.Areas.Admin.Controllers
{
    ...
    public class AdminClaimsController : Controller
    {
        ...
        public async Task<IActionResult> EditUser(string id)
        {
            ...
            if (user == null)
            {
                ModelState.AddModelError("", "Usuário não encntrado");
                return View();
            }
        ...
        {
            //aqui vou localizar o usuário
            var user = await _userManager.FindByIdAsync(model.Id);

            //testo para ver se esta null
            if (user is null)
            {
                //se sim , eu adiciono o erro ao ModelState
                ModelState.AddModelError("", "Usuário não encontrado");
                return View(model);
            }
            else
            {
                //caso contrario e atualizo os dados do usuário
                user.Email = model.Email;
                user.UserName = model.UserName;
                var result = await _userManager.UpdateAsync(user);
                // se atualizado com sucesso e redireciono para a index
                if (result.Succeeded)
                {
